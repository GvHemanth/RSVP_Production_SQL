...
SELECT 
    COUNT(movie_id) AS Rows_in_dirctMapping
FROM
    director_mapping;
SELECT 
    COUNT(movie_id) AS Rows_in_Genre
FROM
    genre;
SELECT 
    COUNT(id) AS Rows_in_movie
FROM
    movie;
SELECT 
    COUNT(id) AS Rows_in_names
FROM
    names;
SELECT 
    COUNT(movie_id) AS Rows_in_ratings
FROM
    ratings;
SELECT 
    COUNT(name_id) AS Rows_in_role_mapping
FROM
    role_mapping;
...
SELECT 
    ...
    SUM(CASE
        ...
        ELSE 0
    END) AS Nulls_in_Year,
    SUM(CASE
        WHEN date_published IS NULL THEN 1
        ELSE 0
    END) AS Nulls_in_Date_Published,
    SUM(CASE
        WHEN duration IS NULL THEN 1
        ELSE 0
    END) AS Nulls_in_Duration,
    SUM(CASE
        WHEN country IS NULL THEN 1
        ELSE 0
    END) AS Nulls_in_Country,
    SUM(CASE
        WHEN worlwide_gross_income IS NULL THEN 1
        ELSE 0
    END) AS Nulls_in_WorldWide,
    SUM(CASE
        WHEN languages IS NULL THEN 1
        ELSE 0
    END) AS Nulls_in_Language,
    SUM(CASE
        WHEN production_company IS NULL THEN 1
        ELSE 0
    END) AS Nulls_in_Prod_Company
FROM
    movie;
...
SELECT 
    median_rating, count(title) as No_of_Movies
FROM
    movie AS m
        INNER JOIN
    ratings AS r ON m.id = r.movie_id
...
SELECT 
    n.name AS actor_name, COUNT(movie_id) as No_of_Movies
FROM
    names AS n
        INNER JOIN
...
SELECT title,
       ...
       CASE
         ...
         WHEN avg_rating BETWEEN 5 AND 7 THEN "one-time_watch movies"
         ELSE "flop movies"
       END        AS Category
FROM   ratings AS r
       INNER JOIN movie AS m
...
(
         ...
         FROM     genre
         GROUP BY genre
         ORDER BY movie_count DESC limit 3 ), movie_sum AS
(
           SELECT     g.genre,
                      m.year,
                      m.title                                                                    AS movie_name,
                                 Concat("$ ",m.worlwide_gross_income)                            AS world_gross_income,
                      Dense_rank () OVER (partition BY year ORDER BY worlwide_gross_income DESC) AS movie_rank
           FROM       (
                               SELECT   title ,
                                        id,
                                        year,
                                        Sum(worlwide_gross_income) AS worlwide_gross_income
                               FROM     (
                                               SELECT title,
                                                      id,
                                                      year,
                                                      CASE
                                                             WHEN currency="US" THEN modified_income
                                                             WHEN currency="IND" THEN modified_income/80
                                                             ELSE 0
                                               ...
                                               FROM   (
                                                             SELECT title,
                                                                    ...
                                                                    CASE
                                                                           WHEN worlwide_gross_income LIKE "$%" THEN "US"
                                                                           WHEN worlwide_gross_income LIKE "INR%" THEN "IND"
                                                                           ELSE "nor"
                                                                    END                                                                                AS currency,
                                                                    Cast(Replace(Replace(Ifnull(worlwide_gross_income,0),'INR',''),'$','') AS DECIMAL) AS modified_income
                                                             FROM   movie) as p ) as q
                               GROUP BY title,
                                        year,
                                        id ) AS m
           INNER JOIN genre                  AS g
...
(
         ...
         ON       dm.name_id=n.id
         JOIN     movie AS m
         ON       dm.movie_id=m.id ), date_difference AS
(
       SELECT *,
              Datediff(next_movie_date, date_published) AS date_diff
       FROM   movie_info ), avg_inter_days AS
(
         SELECT   name_id,
                  Avg(date_diff) AS avg_inter_movie_days
         FROM     date_difference
         GROUP BY name_id ), final AS
(
